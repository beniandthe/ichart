...
- Rhythm Section passed the final fresh app-surface gate for professional staff layout, suppressed header meter, native full-screen surface, the historical V1 rhythm-recognition pass, chord snapping, below-staff freehand, structured chart objects, and save/reopen behavior. The current shipping boundary supersedes that recognizer path: V1 uses Free-Write for rhythm notation, and V1.1 targets select-input rhythm notation.
