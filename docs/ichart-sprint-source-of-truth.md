...
## Current V1.1 Promise

The current post-V1 product target is defined in `docs/ichart-v1-1-roadmap.md`.
V1.1 will focus on:

- Key signatures.
- Key-aware enharmonic chord spelling.
...
are explicitly carried into the V1.1 roadmap.
...
- latest Sprint 69 closeout: Sprint 69 is complete locally. The final fresh Rhythm Section app-surface pass was green for professional staff layout, suppressed header meter, native full-screen surface, the then-current rhythm-recognition pass, chord snapping with and without rhythm maps, below-staff freehand articulation, structured cue/roadmap/repeat objects, and save/reopen behavior. That rhythm-recognition evidence is now superseded by the V1 rhythm-recognition parking decision: V1 uses Free-Write for rhythm notation, and select-input rhythm notation is parked for V1.2. No V1 blocker remains confirmed by the audit. The readiness matrix now moves the next lane to Sprint 70 App Functionality and UX/UI, starting with project/library UX, export/share confidence, save/reopen confidence, and tool/menu affordance polish.
- latest Sprint 70 chord transposition checkpoint: chart-wide chord transposition is implemented locally as a one-time chord rewrite action. Page > Transpose exposes reset, up/down half-step, and direct half-step actions; each nonzero action mutates the existing rendered chord symbols and then clears `Chart.chordTranspositionSemitones` back to `0` so future handwritten/manual chords stay exactly as entered. Legacy charts that still decode with a nonzero `chordTranspositionSemitones` value normalize on load by applying that stored offset once to existing chord events and clearing the offset. `Chart.displayedChordSymbol(for:)` remains the shared display authority for instrument transposition and any legacy in-memory offset, so live canvas and PDF/export paths still share one rendered-symbol path. Focused SwiftPM `swift test --scratch-path /tmp/iChartSwiftBuild-transpose-chartediting --filter ChartEditingTests` passed with `117` tests and `0` failures; focused SwiftPM `swift test --scratch-path /tmp/iChartSwiftBuild-transpose-layout --filter 'LeadSheetPageLayoutTests/testSimpleChordSheetRendersOneTimeAppliedChordTransposition|LeadSheetPageLayoutTests/testRhythmSectionSheetRendersOneTimeAppliedChordTranspositionWithoutMovingSnaps'` passed with `2` tests and `0` failures.
...
- latest account/cloud monetization decision: Basic remains a mandatory account tier with the complete local chart-writing tool, PDF/export, local autosave, and a 3-chart local cap. Pro is a monthly/annual subscription surface for unlimited chart capacity, cloud chart backup/sync/restore, Forums, and future service-heavy features. If Pro renewal is canceled, full Pro remains active through the paid-through date. Apple billing grace preserves local chart access while cloud backup and Forums pause. If Pro expires, cloud sync and Forums lock, remote chart backups are deleted after the paid-through date or Apple billing grace deadline unless Pro renews first, and over-cap local libraries lock opening/editing until the user chooses 3 Basic charts. This downgrade pruning is local-only and does not create cloud deletion tombstones.
...
- Rhythm Section passed the final fresh app-surface gate for professional staff layout, suppressed header meter, native full-screen surface, the historical V1 rhythm-recognition pass, chord snapping, below-staff freehand, structured chart objects, and save/reopen behavior. The current shipping boundary supersedes that recognizer path: V1 uses Free-Write for rhythm notation, and select-input rhythm notation is parked for V1.2.
...
- The handwritten Rhythm tool is retired for the shipping app. Rhythm Section charts keep staff layout, measure systems, chord placement, repeats, text, export, and Free-Write; dedicated rhythm-map input will return later as a literal selector/palette, not handwriting recognition.
- `RhythmicNotationCompendium` gates committed rhythm maps to the supported v1 rhythm vocabulary and exact meter fit.
- Free-Write is the current rhythm-entry path. It keeps user ink exactly as written and does not analyze, clear, or write rhythm maps from handwriting.
- The parked recognizer research is no longer a user-facing entry path. `RhythmRecognitionOverhaulGate.shipsDedicatedRhythmTool` disables the dedicated toolbar tool and direct rhythmic notation ink scope until the literal input system is ready.
...
- Existing rhythm-map model/render/export structures remain available for saved data and future literal input. Handwritten recognition files and references are parked as research, not launch behavior.
...
- Historical Lead Sheet baseline work rendered selected treble/bass clefs, placed transposed key-signature accidentals before the first measure's time signature, and proved a narrow pitched-entry path. Lead Sheet remains archived under `docs/post-v1/lead-sheet/`; key signatures for the active product are now a V1.1 roadmap item, and melody/lyric lead-sheet engraving is not part of the V1.1 promise unless explicitly reprioritized.
- User-facing rhythm-note editing, freehand symbol ink availability, and rhythmic-notation ink availability remain profile-owned contracts, but the shipping editor now gates direct rhythmic notation ink off globally until literal rhythm input is implemented.
...
- tests and evidence: Sprint 69 focused and grouped verification is recorded in `docs/ichart-sprint-69-v1-readiness-audit-2026-05-31.md`. Key final evidence includes historical XcodeBuildMCP `RhythmicNotationQuantizerTests` with `83` passed, `1` optional replay skipped, and `0` failures after the direct whole-note-circle rule; V1 persistence/store proofs for Simple and Rhythm Section; repeated `git diff --check` passes; fresh simulator uninstall/reinstall build/run with the existing headermap warning only; and the final user-reported Rhythm Section app-surface pass green for layout, the then-current rhythm-recognition path, chord snapping, below-staff freehand, structured chart objects, and save/reopen. The current V1 release boundary supersedes that recognizer path and ships rhythm notation through Free-Write until the V1.2 select-input system is built.
