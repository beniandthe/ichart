# iChart Sprint 71 V1 Product Readiness And Release Hardening
...
- Keep the Charts tab work-first: no redundant top header, a centered `New Chart` action, Basic chart usage only when the user is capped, readable chart rows, and user-selectable row preview density.
...
- The Charts tab now centers the `New Chart` action at the top, shows Basic chart usage only when a local chart cap applies, and keeps chart rows simple by default.
...
- Slice 1 focused SwiftPM: `swift test --scratch-path /tmp/iChartSwiftBuild-layoutprofile --filter ChartLibraryStoreTests`
...
- `git diff --check` passed.
- GitHub Actions branch run [`26922352743`](https://github.com/beniandthe/ichart/actions/runs/26922352743) passed on `fc46d33`: `SwiftPM tests` passed in `1m46s`, and `iOS simulator tests` passed in `8m7s`.
...
- Focused XcodeBuildMCP simulator store group passed with `26` tests and `0` failures: `iChartTests/ChartLibraryStoreTests`.
...
- Focused SwiftPM `swift test --scratch-path /tmp/iChartSwiftBuild-layoutprofile --filter ChartLibraryStoreTests` passed with `27` tests and `0` failures.
- Full SwiftPM `swift test --scratch-path /tmp/iChartSwiftBuild-layoutprofile` passed with `494` tests, `36` skipped, and `0` failures.
- `git diff --check` passed.
...
- Chord-entry timer checkpoint: idle recognition now acts only as a quiet preview/cache path. Chord rendering and confirmation-sheet presentation require an explicit user commit tap from Chord mode, and that commit path bypasses continuation grace so the user is not racing the idle timer or waiting through a second timer after tapping out. This is an interaction-policy change only: no recognition scores, OCR, parser/compendium authority, fixture set, chord placement, or correction-memory policy changed.
- Chord-entry cleanup checkpoint: the chord paths are now split and named as `automaticPreview` and `tapToConfirm`, with automatic scheduling, tap-confirm gesture gating, and render-resolution/memory decisions living in separate policy types. Deprecated aliases remain for old names only as legacy breadcrumbs. Verification passed: focused XcodeBuildMCP simulator chord/editor policy tests with `56` tests and `0` failures; full SwiftPM with `501` tests, `36` skipped, and `0` failures; `git diff --check`; XcodeBuildMCP `build_run_sim CODE_SIGNING_ALLOWED=NO`; simulator screenshot of the iChart home shell.
