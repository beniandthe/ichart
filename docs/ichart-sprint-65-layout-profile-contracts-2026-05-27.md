# iChart Sprint 65 Layout Profile Contracts
...
Sprint 65 gives each `ChartLayoutStyle` a model-level profile contract before iChart branches any renderer, export, or editor behavior by layout.
...
- `swift test --scratch-path /tmp/iChartSwiftBuild-layoutprofile --filter ChartEditingTests` passed with `35` tests, `0` failures.
- `swift test --scratch-path /tmp/iChartSwiftBuild-layoutprofile` passed with `336` tests, `36` skipped, `0` failures.
- `xcodegen generate` completed; XcodeBuildMCP `build_sim CODE_SIGNING_ALLOWED=NO` passed for the `iChart` scheme on the configured iPad simulator.
- The generated ignored `iChart.xcodeproj` was removed after simulator verification.
