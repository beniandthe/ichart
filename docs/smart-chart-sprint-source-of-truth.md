...
- Cohesion polish is implemented locally: Simple Chord Sheet now reserves a left meter gutter across every rendered row and draws the initial time signature before the first measure; Simple trailing meter changes are visible in the grid instead of being suppressed; the active Measure menu stays reopenable during pending repeat/ending workflows instead of toggling back to `Select`; and header/metadata/annotation text uses stable app-standard fonts so changing notation fonts does not clip chart headers.
