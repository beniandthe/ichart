...
- Project/library actions are the first slice.
- Rename, duplicate, and delete should be model-backed through the library store/repository authority.
- Duplicate must create a fresh chart identity with copied content and a clear duplicate title.
- Delete must remove saved chart data and clean selected-chart references.
- Developer-facing library behavior should be hidden or gated from the production-facing surface.
...
5. Toolstrip/menu affordances: only targeted polish not covered by the user-side UI overhaul.
