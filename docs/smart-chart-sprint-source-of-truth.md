...
- latest Sprint 70 live ink responsiveness checkpoint: passive ink responsiveness is implemented locally for handwritten headers, page/freehand ink, and freehand-symbol writing. `LeadSheetInkCanvasSyncPolicy` now preserves dirty passive ink canvases from stale model reloads, matching the existing chord/rhythm dirty-canvas guard pattern. Passive ink persists only after a stable idle window or when leaving the tool, and mode-exit persistence now uses the previous tool's active ink scope instead of resolving after the editor has already returned to Select. This keeps fast handwriting live in `PKCanvasView` longer and avoids mid-writing serialization/redraw churn. Chord recognition, rhythm recognition, OCR, score thresholds, default diagnostics, and personal handwriting fixtures remain unchanged. Focused SwiftPM `swift test --scratch-path /tmp/SmartChartSwiftBuild-layoutprofile --filter 'LeadSheetInteractionModeStatePolicyTests|ChartEditingTests|LeadSheetPageLayoutTests|FileChartRepositoryTests'` passed with `174` tests and `0` failures; full SwiftPM `swift test --scratch-path /tmp/SmartChartSwiftBuild-layoutprofile` passed with `467` tests, `36` skipped, and `0` failures; `git diff --check` passed; a fresh wiped iPad Pro 13-inch (M5) simulator build/run via XcodeBuildMCP `build_run_sim CODE_SIGNING_ALLOWED=NO` succeeded with the existing headermap warning only and screenshot capture completed after launch.
