...
- Sprint 69 V1 readiness audit artifact: `docs/smart-chart-sprint-69-v1-readiness-audit-2026-05-31.md`
- Sprint 69 V1 readiness matrix: `docs/smart-chart-v1-readiness-matrix-2026-05-31.md`
- Sprint 70 App Functionality and UX/UI artifact: `docs/smart-chart-sprint-70-app-functionality-ux-2026-06-02.md`
- Sprint 71 V1 Product Readiness and Release Hardening artifact: `docs/smart-chart-sprint-71-v1-product-readiness-2026-06-04.md`
...
- latest account/cloud product decision: iChart should require recoverable accounts for both Basic and Pro users. Account/auth/email verification/password recovery/profile/subscription identity are base trust infrastructure, not paid features. Chart authoring remains local-first. Cloud chart backup/sync/restore is the Pro subscription cloud-service surface and must be gated by active Pro entitlement before production rollout. The current signed-in chart sync behavior is an interim QA path until StoreKit/subscription entitlement wiring is implemented. Domain/custom SMTP setup is still pending before hosted Auth email templates can use the final direct app recovery link.
...
- The V1 readiness matrix is recorded at `docs/smart-chart-v1-readiness-matrix-2026-05-31.md`.
...
- Rhythm Recognition V2 started as an auto-render safety gate: exact meter fit is still required, but no longer sufficient for live ink clearing. A proposal carries `autoApply`, `extendedStability`, or `manualReview` safety, and the host commits only proposals that can auto-apply. Current V4 evidence now allows strong single-crop whole-note proposals to auto-render, while tiny low-information whole-like marks and single whole-rest measures remain review-only.
...
- tests and evidence: focused and grouped SwiftPM/simulator verification was recorded throughout `docs/smart-chart-sprint-68-chart-structure-systems-definition-2026-05-29.md`; the final Simple typography/layout checkpoint passed full `swift test --scratch-path /tmp/SmartChartSwiftBuild-layoutprofile` with `437` tests, `36` skipped, and `0` failures; `git diff --check` passed; fresh simulator wipe plus XcodeBuildMCP `build_run_sim CODE_SIGNING_ALLOWED=NO` succeeded with the existing headermap warning only; the user live pass confirmed the core loop is working and looking as intended.
...
- summary: Moved rhythmic notation confirmation pending state and sheet UI out of `EditorView.swift` into `SmartChart/Features/Editor/Components/RhythmicNotationConfirmationSheetView.swift`. Removed the rhythm confirmation label helper from `EditorView.swift` and kept the existing shared `FlowLayout` component. This reduced `EditorView.swift` from roughly `1683` lines to roughly `1544` lines while keeping editor orchestration in place. This historical Sprint 19 confirmation path is now superseded by the 2026-06-01 Rhythm Section live route cleanup, which removes the rhythm confirmation sheet from the active runtime contract.
