...
- Simple Chord Sheet visual/tooling polish is implemented locally: the `Rhythmic Notation` tool is hidden entirely for Simple while remaining available for Rhythm Section and Lead Sheet; Simple measure bodies are taller chord-grid cells with stronger barlines for a clearer iReal/handwritten-chart read; and the rendered header is now a centered title with one compact metadata row for style, meter/key, and composer, with the old title underline removed.
- Cohesion polish is implemented locally: Simple Chord Sheet now reserves a left meter gutter across every rendered row and draws the initial time signature before the first measure; non-initial time changes render inline inside the measure whose meter they change; the active Measure menu stays reopenable during pending repeat/ending workflows instead of toggling back to `Select`; header/metadata/annotation text uses stable app-standard fonts so changing notation fonts does not clip chart headers; and the first shared `VisualPolicy` seam now owns common layout constants instead of scattering them through the engine.
- Latest screenshot-informed Simple cohesion pass is implemented locally: Simple Chord Sheet now leans further into an iReal-style chord-chart feel without cloning it. The Simple rendered page uses a compact title-case header, style note on the left, composer/credit on the right, no header key/meter block, white paper, larger bold size-fitting chord text inside the grid, and black section badges sized to their label text. The initial time signature remains before the first measure in the shared left gutter; later time changes stay inline inside the changed measure.
- Latest Simple chord-fit and typography pass is implemented locally: Simple chord layout now gives rendered chords an invisible snapped measure segment for placement authority. A lone beat-1 chord keeps that full-measure snap segment but renders inside a capped left-side primary-beat visual slot, leaving beat-3 writing room for the next chord. One- and two-chord measures keep a fixed uncompressed chord scale regardless of the selected beat positions, so moving the second chord around the bar cannot squeeze or expand either chord. Measures with three or more chords now lay out the whole chord row from natural chord widths first, then distribute the remaining barline-to-barline measure space as equal gaps around and between chords; only if that full row still cannot fit does the measure apply one shared horizontal compression scale across every chord. That scale is count/content based, not beat-position based, so a beat-4 chord or a moved later chord cannot be compressed differently from the earlier chords in the same three-or-more-chord measure. Later appends honor the written side of an existing later-beat chord instead of silently jumping back to the first empty preferred beat. The visible chord frame and edit box wrap the rendered chord text instead of looking like a segment resize box. Page > Fonts now opens role-based typography settings: a matched set plus Chord Font, Header Font, Text / Cue Font, and Notation Symbols roles. `ChartTypographySettings` preserves legacy `notationFont` compatibility, matched sets update notation-symbol defaults, and role overrides affect only their own role. Simple chord rendering now tokenizes structured `ChordSymbol` values into root/accidental text, suffix text, music-symbol quality marks, alterations, and slash bass; roots keep text-style `b/#` accidentals, Finale Jazz uses the lowercase-safe chord text face, and triangle/diminished/half-diminished marks fall back to symbol/vector drawing instead of disappearing when a text face lacks a glyph. Simple chord typography now has one shared size contract across chord font roles: 46-point primary text, 54% suffix/symbol text, a shared 2-point token gap, and a final renderer clamp so wider real font glyphs cannot overflow their resolved measure segment. MuseJazz is bundled from the official MuseScore source with OFL license and SMuFL metadata alongside the existing Bravura, Petaluma, Leland, and Finale families.
...
- Focused Simple header/grid polish verification: focused SwiftPM `LeadSheetPageLayoutTests` passed with `55` tests and `0` failures; full `swift test --scratch-path /tmp/SmartChartSwiftBuild-layoutprofile` passed with `414` tests, `36` skipped, and `0` failures; XcodeBuildMCP focused simulator `LeadSheetInteractionModeStatePolicyTests` and `LeadSheetPageLayoutTests` passed with `24` and `55` tests respectively; `git diff --check` passed; XcodeBuildMCP `build_run_sim CODE_SIGNING_ALLOWED=NO` and screenshot capture succeeded with the existing headermap warning only.
- Focused cohesion polish verification: focused SwiftPM `LeadSheetPageLayoutTests` passed with `57` tests and `0` failures after adding Simple meter-gutter and Simple meter-change layout coverage; focused SwiftPM `ChartEditingTests/testDraftStoresSelectedLayoutStyleAndDefaults` passed with `1` test and `0` failures.
- Focused inline meter-change verification: focused SwiftPM `LeadSheetPageLayoutTests` passed with `57` tests and `0` failures after changing non-initial meter changes to render inside the changed measure; focused SwiftPM `ChartEditingTests/testApplyMeterChangeToNextTimeSignaturePersistsIntoFutureMeasures` passed with `1` test and `0` failures.
...
- Latest screenshot-informed cohesion verification: focused SwiftPM `LeadSheetPageLayoutTests` passed with `58` tests and `0` failures; full SwiftPM verification passed with `417` tests, `36` skipped, and `0` failures; XcodeBuildMCP focused simulator `test_sim -only-testing:SmartChartTests/LeadSheetPageLayoutTests -only-testing:SmartChartTests/PDFChartExporterTests CODE_SIGNING_ALLOWED=NO` passed with `63` tests and `0` failures; XcodeBuildMCP `build_run_sim CODE_SIGNING_ALLOWED=NO` succeeded with the existing headermap warning only, and screenshot capture succeeded.
- Latest Simple chord-fit verification: focused SwiftPM `LeadSheetPageLayoutTests` passed with `64` tests and `0` failures after adding the two-chord no-compression-on-move regression and the three-or-more-chords even-slot regression; focused SwiftPM `ChartEditingTests/testSimpleChordSheetAppendAfterExistingLaterBeatKeepsWrittenOrder` passed with `1` test and `0` failures; full SwiftPM verification passed with `427` tests, `36` skipped, and `0` failures; `git diff --check` passed; XcodeBuildMCP focused simulator `test_sim -only-testing:SmartChartTests/ChartEditingTests/testSimpleChordSheetAppendAfterExistingLaterBeatKeepsWrittenOrder -only-testing:SmartChartTests/LeadSheetPageLayoutTests -only-testing:SmartChartTests/LeadSheetInteractionModeStatePolicyTests CODE_SIGNING_ALLOWED=NO` passed with `89` tests and `0` failures; XcodeBuildMCP `build_run_sim CODE_SIGNING_ALLOWED=NO` and screenshot capture succeeded with the existing headermap warning only. Follow-up current page-font chord rendering verification: full SwiftPM verification passed again with `427` tests, `36` skipped, and `0` failures; `git diff --check` passed; XcodeBuildMCP `build_run_sim CODE_SIGNING_ALLOWED=NO` succeeded with the existing headermap warning only; screenshot capture confirmed the simulator launched. Follow-up chord-safe Finale Jazz mapping verification: focused SwiftPM `ChartEditingTests/testFinaleJazzUsesLowercaseSafeChordTextFace` passed with `1` test and `0` failures before full verification. Follow-up role-based typography verification: focused SwiftPM `ChartTypographyResolverTests` passed with `6` tests and `0` failures; focused SwiftPM `ChartEditingTests`, `LeadSheetPageLayoutTests`, and `ChartTypographyResolverTests` passed with `158` tests and `0` failures; full `swift test --scratch-path /tmp/SmartChartSwiftBuild-layoutprofile` passed with `434` tests, `36` skipped, and `0` failures; `git diff --check` passed; XcodeBuildMCP `build_run_sim CODE_SIGNING_ALLOWED=NO` succeeded with the existing headermap warning only and screenshot capture succeeded. Follow-up universal Simple chord text-size verification: focused SwiftPM `ChartTypographyResolverTests` passed with `8` tests and `0` failures; focused SwiftPM `LeadSheetPageLayoutTests/testSimpleChordSheetChordFramesUseUniversalTypographyAcrossChordFonts` passed with `1` test and `0` failures; grouped SwiftPM `LeadSheetPageLayoutTests` plus `ChartTypographyResolverTests` passed with `73` tests and `0` failures; full SwiftPM verification passed with `437` tests, `36` skipped, and `0` failures; `git diff --check` passed; clean simulator wipe plus XcodeBuildMCP `build_run_sim CODE_SIGNING_ALLOWED=NO` succeeded with the existing headermap warning only and screenshot capture succeeded.
- Latest Simple chord edit-box verification: focused SwiftPM `LeadSheetPageLayoutTests` passed with `60` tests and `0` failures; full SwiftPM verification passed with `419` tests, `36` skipped, and `0` failures; XcodeBuildMCP focused simulator `test_sim -only-testing:SmartChartTests/LeadSheetPageLayoutTests -only-testing:SmartChartTests/LeadSheetChordEditOverlayGeometryTests -only-testing:SmartChartTests/PDFChartExporterTests CODE_SIGNING_ALLOWED=NO` passed with `69` tests and `0` failures; `git diff --check` passed; clean simulator reinstall/rebuild and screenshot capture succeeded.
