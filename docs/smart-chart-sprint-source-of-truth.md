...
- The Charts tab now drops the redundant top header, centers `New Chart`, shows free-tier chart usage only for capped users, simplifies chart rows, and lets the user switch row previews between `Collapsed`, `Quick`, and `Large` modes.
