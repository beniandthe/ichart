...
6. Open the verification email. If the browser opens `https://useichart.com/verify.html` or the rewritten `https://useichart.com/verify` route, confirm it offers a clear Open iChart handoff and does not display token values. If the app opens directly, return to iChart and sign in with the same email/password.
