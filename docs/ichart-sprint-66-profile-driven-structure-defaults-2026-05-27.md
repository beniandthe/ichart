# iChart Sprint 66 Profile-Driven Structure Defaults
...
- This layout is the current basic chart workflow iChart has been building so far.
...
- `swift test --scratch-path /tmp/iChartSwiftBuild-layoutprofile --filter ChartEditingTests` passed with `35` tests, `0` failures after updating the Simple Chord Sheet profile to default to one starting measure.
...
- `swift test --scratch-path /tmp/iChartSwiftBuild-layoutprofile --filter LeadSheetPageLayoutTests` passed with `35` tests, `0` failures after adding Lead Sheet key-signature layout, bass-clef position, and Rhythm Section no-key-signature guards.
...
- Full `swift test --scratch-path /tmp/iChartSwiftBuild-layoutprofile` passed with `356` tests, `36` skipped, and `0` failures after the Lead Sheet key-signature baseline.
...
- `swift test --scratch-path /tmp/iChartSwiftBuild-layoutprofile --filter ChartEditingTests --filter LeadSheetPageLayoutTests` passed with `84` tests and `0` failures after the Lead Sheet pitched-note model/layout baseline.
...
- Full `swift test --scratch-path /tmp/iChartSwiftBuild-layoutprofile` passed with `360` tests, `36` skipped, and `0` failures after the Lead Sheet pitched-note baseline.
...
- `swift test --scratch-path /tmp/iChartSwiftBuild-layoutprofile --filter ChartEditingTests --filter LeadSheetPageLayoutTests` passed with `86` tests and `0` failures after adding mixed Lead Sheet note/rest commit and layout coverage.
...
- Full `swift test --scratch-path /tmp/iChartSwiftBuild-layoutprofile` passed with `362` tests, `36` skipped, and `0` failures after the mixed Lead Sheet note/rest plus beamed-eighth pitch-anchor slice and again after the pitch-coverage guard.
...
- `swift test --scratch-path /tmp/iChartSwiftBuild-layoutprofile --filter ChartEditingTests` passed with `48` tests and `0` failures after moving user-facing rhythm-note editing availability into `ChartLayoutProfile`; grouped simulator `test_sim` passed with `186` tests and `0` failures.
...
- `swift test --scratch-path /tmp/iChartSwiftBuild-layoutprofile --filter ChartEditingTests` passed with `48` tests and `0` failures after moving rhythmic-notation ink availability into `ChartLayoutProfile`.
...
- Full `swift test --scratch-path /tmp/iChartSwiftBuild-layoutprofile` passed with `362` tests, `36` skipped, and `0` failures after the rhythmic-notation availability guard.
