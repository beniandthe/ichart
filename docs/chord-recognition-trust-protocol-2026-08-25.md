...
- 2026-09-05 aggregate issue-bucket telemetry slice:
  - Trigger: production telemetry was queried across all users after the local device-trace work. The DB showed `chord.preview_updated` events are logging and privacy-safe, but the existing aggregate counters can only show matched/trusted/confirm/no-read rates. They cannot say whether current friction is concentrated in roots, flats/sharps, triangle-vs-dim quality, extensions, alterations, slash bass, or barline sequences.
  - Implementation contract: keep using `chord.preview_updated`; do not create a raw handwriting event. Add only aggregate issue-family counts derived from in-memory recognizer results before telemetry sanitization.
  - Added aggregate fields: `issue_count`, `root_issue_count`, `root_accidental_issue_count`, `quality_issue_count`, `triangle_quality_issue_count`, `dim_quality_issue_count`, `extension_issue_count`, `alteration_issue_count`, `slash_bass_issue_count`, `barline_sequence_issue_count`, `candidate_limit_issue_count`, and `unknown_issue_count`. Family buckets may overlap; `issue_count` is the number of issue targets, not the sum of all family counters.
  - Privacy boundary: these fields must remain counts only. Production telemetry must not contain raw chord text, candidate text, glyph columns, stroke points, `PKDrawing` payloads, chart titles, user names, emails, correction text, or support content.
  - Product boundary: this slice does not change recognition, grouping, candidate scoring, trust policy, preview rendering, Render Chords behavior, lane UI, edit mode, ink persistence, sync, resize, terminal behavior, OCR, Scribble, or `PKStrokeRecognizer`.
  - Operational use: bucket trends are for prioritizing the next recognizer fixture and architecture work. A spike in `triangle_quality_issue_count` or `slash_bass_issue_count` tells us where to inspect DEBUG traces or promote labeled fixtures; it is not enough by itself to assert a specific wrong read.
  - Verification requirement before deployment: app privacy tests, bucket classifier tests, Supabase ingest tests, `xcodegen generate`, and `git diff --check` must pass. Production deployment of the Edge Function remains a separate explicit action.
